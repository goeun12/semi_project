...
insert into member values(seq_member.nextval, '라라라', 'user04', 'pass04', '01044445555', default, default, sysdate);
insert into member values(seq_member.nextval, '관리자', 'admin', 'admin123', '01055556666', 'Y', default, sysdate);

insert into board values(seq_board.nextval, '레시피', 100, '떡라면', '냄비에 물을 붓고 끓인다.§§●스프를 끓는 물에 넣는다.§§●떡과 라면, 후레이크를 넣고 끓인다.', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '꿀팁', 100, '라면에 계란 넣어서 먹어봐', '근데 넣고 풀지말고 냄비 가운데에 넣고 면으로 덮어둔 다음에 조금 끓이고 먹으면 치즈라면 맛남', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '꿀팁', 101, '제로 탕후루 진짜 맛있었음', '얼음 띄운 물에 그냥 냉동과일 넣어서 얼려 먹는건데 탕후루 식감나서 좋았어. 설탕도 안들어있으니까 탕후루 먹고 싶으면 이렇게 하면 될듯', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '공지', 104, '혼밥심 사이트 동작이 시작되었습니다.', '안녕하세요, 관리자 admin입니다. 저희 혼밥심은 혼자서 식사할 때 저렴한 비용, 짧은 조리 시간 등을 고려한 식사 레시피를 올리는 곳입니다. 그 외에도 무엇을 먹는지 추천하는 랜덤 요리 메뉴와 사용자 간의 커뮤니케이션을 위해 꿀팁 게시판이라는 커뮤니티를 열어두고 있으니 많이 참여해주세요.', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '레시피', 103, '계란말이', '계란 3개를 풀어낸 뒤 소금 2꼬집과 후추 1꼬집을 넣는다.§§●팬에 기름을 바른 뒤, 중약불로 계란물을 얇게 붓는다.§§●계란물이 익으면 돌돌 말아낸 뒤, 다시 계란물을 부어 반복한다.', sysdate, sysdate, default);

insert into recipe values(seq_recipe.nextval, 7, '맛있는 라면에 떡사리를 추가해 더욱 맛있게 먹을 수 있다.', '한식', '하', '봉지 라면, 떡');
insert into recipe values(seq_recipe.nextval, 11, '포슬포슬한 계란말이다. 맛있다.', '한식', '하', '계란 3개, 소금, 후추');
