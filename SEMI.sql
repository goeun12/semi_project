...
insert into board values(seq_board.nextval, '레시피', 106, '떡라면', '냄비에 물을 붓고 끓인다.§§●스프를 끓는 물에 넣는다.§§●떡과 라면, 후레이크를 넣고 끓인다.', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '꿀팁', 108, '라면에 계란 넣어서 먹어봐', '근데 넣고 풀지말고 냄비 가운데에 넣고 면으로 덮어둔 다음에 조금 끓이고 먹으면 치즈라면 맛남', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '꿀팁', 107, '제로 탕후루 진짜 맛있었음', '얼음 띄운 물에 그냥 냉동과일 넣어서 얼려 먹는건데 탕후루 식감나서 좋았어. 설탕도 안들어있으니까 탕후루 먹고 싶으면 이렇게 하면 될듯', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '공지', 109, '혼밥심 사이트 동작이 시작되었습니다.', '안녕하세요, 관리자 admin입니다. 저희 혼밥심은 혼자서 식사할 때 저렴한 비용, 짧은 조리 시간 등을 고려한 식사 레시피를 올리는 곳입니다. 그 외에도 무엇을 먹는지 추천하는 랜덤 요리 메뉴와 사용자 간의 커뮤니케이션을 위해 꿀팁 게시판이라는 커뮤니티를 열어두고 있으니 많이 참여해주세요.', sysdate, sysdate, default);
insert into board values(seq_board.nextval, '레시피', 105, '계란말이', '계란 3개를 풀어낸 뒤 소금 2꼬집과 후추 1꼬집을 넣는다.§§●팬에 기름을 바른 뒤, 중약불로 계란물을 얇게 붓는다.§§●계란물이 익으면 돌돌 말아낸 뒤, 다시 계란물을 부어 반복한다.', sysdate, sysdate, default);
